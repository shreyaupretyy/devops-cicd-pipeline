...
Figure \ref{fig:jenkins} shows the Jenkins web interface running locally on port 8080 after installation and initial configuration. The pipeline is defined in the \texttt{Jenkinsfile} at the root of the repository and is triggered via a GitHub webhook on every push to the \texttt{main} branch.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{jenkins_dashboard.png}
\caption{Jenkins web interface running on port 8080}
\label{fig:jenkins}
\end{figure}

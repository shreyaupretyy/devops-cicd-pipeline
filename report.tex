...
\begin{enumerate}
    ...
    \item On successful tests, the pipeline shall build a Docker image tagged with the Jenkins build number and \texttt{latest}.
...
\begin{tikzpicture}[node distance=1.4cm and 2cm]
    ...
    \node[block, right=of dockerbuild] (ansible) {Ansible};
    ...
    \draw[arrow] (dockerbuild) -- node[above,font=\scriptsize]{deploy} (ansible);
    \draw[arrow] (ansible) -- node[right,font=\scriptsize]{start} (app);
...
The Jenkinsfile implements a declarative pipeline with five stages:
...
\begin{tikzpicture}[node distance=0.5cm]
    ...
    \node[block, below=of build, fill=orange!20] (deploy) {Ansible Deploy};
...
Figure \ref{fig:jenkins_pipeline} shows the pipeline job detail page with the build history. Build \#5 completed all five stages with a green \texttt{SUCCESS} result: Checkout, Install Dependencies, Run Tests (all 5 Jest tests passed), Build Docker Image, and Deploy via Ansible (playbook syntax validated).
...
The declarative Jenkinsfile (Listing \ref{lst:jenkinsfile}) defines the Docker build stage which tags the image with both the build number and \texttt{latest} for traceability.

\begin{lstlisting}[caption={Jenkinsfile: Build Docker Image stage}, label={lst:jenkinsfile}]
stage('Build Docker Image') {
    ...
    post {
        always {
            echo "==> Build stage complete: ${currentBuild.currentResult}"
        }
    }
}
\end{lstlisting}
...
\section{Summary}
This project designed and implemented a complete, production-aware CI/CD pipeline for a containerised Node.js To-Do application. The pipeline integrates GitHub, Jenkins, Docker, Ansible, Prometheus, and Grafana into a cohesive automated system. Key achievements include:

\begin{enumerate}
    \item Automated build and test execution on every code push, with test failures preventing deployment.
    \item Reproducible Docker images built locally and deployed via idempotent Ansible playbooks.
